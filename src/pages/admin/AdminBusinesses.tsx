...
export default function AdminBusinesses() {
  ...
  useEffect(() => { apiGet('/api/admin?resource=business').then(d => { setList(d); setLoading(false); }).catch(() => setLoading(false)); }, []);
  if (loading) return <Spinner />;
  return (
    <div>
      <PageHeader title="Businesses" subtitle={`${list.length} providers on the platform.`} />
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {list.map(b => (
          <div key={b.id} className="card overflow-hidden">
            <img src={b.image_url} alt={b.name} className="h-32 w-full object-cover" />
