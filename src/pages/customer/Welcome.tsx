...
export default function Welcome() {
  ...
  // Native Google OAuth is always available via Supabase when provider enabled; custom proxy is fallback
  ...
  const emailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        // Email confirmation UX: if no session, user must confirm email
        if (!data.session && data.user) {
          // Check if email confirmation is required
          if (!isDemoMode) {
            setInfo(
              'Account created! Check your email to confirm, or turn OFF confirm-email in Supabase Auth settings for instant access. You can also continue as demo.',
            );
            setLoading(false);
            return;
          }
        ...
        if (data.user) {
          supabase
            .from('profiles')
            .upsert({ id: data.user.id, email, full_name: name || email.split('@')[0], role: 'customer' })
            .then(() => {});
        }
  ...
  const demo = async () => {
    ...
    const fullName = role === 'admin' ? 'Demo Admin' : 'Demo Customer';
    try {
      const { error } = await supabase.auth.signInWithPassword({ email: em, password: 'velora123' });
      if (error) {
        // Demo auto-provision: if demo account doesn't exist, create it
        if (/invalid login|user not found|invalid.*credentials/i.test(error.message)) {
          ...
          // Try sign-in again after provisioning
          if (!signUpData.session) {
            const { error: secondErr } = await supabase.auth.signInWithPassword({ email: em, password: 'velora123' });
            if (secondErr) {
              // In demo mode or with confirm-email OFF, signUp already gives session
              if (signUpData.session) {
                nav(homeFor(role || 'customer'), { replace: true });
                return;
              }
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      // For native OAuth, Supabase redirects, so loading stays true until redirect
      // For custom proxy, popup handles session
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative w-full max-w-md text-center">
          ...
          <div className="mt-8 grid gap-4">
            <RoleCard
              ...
              sub="Discover and book appointments near you"
              tint="#6366f1"
              delay={0.05}
            />
            <RoleCard
              ...
              sub="Manage bookings, staff, services & availability"
              tint="#10b981"
              delay={0.12}
            />
          </div>
      ...
      <div className="relative hidden lg:flex flex-col justify-between p-12 overflow-hidden">
        ...
        <div className="relative">
          ...
          <h1 className="text-4xl font-semibold tracking-tight leading-tight">
            {isAdmin ? (
              ...
              <>
                The smartest way
                <br />
                to <span className="grad-text">book anything.</span>
              </>
            )}
          </h1>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {(isAdmin
              ? [
                  ['Live dashboard', CalendarCheck],
                  ['Manage staff', User],
                  ['Instant sync', Sparkles],
                ]
              : [
                  ['Instant booking', CalendarCheck],
                  ['Nearest first', MapPin],
      ...
      <div className="flex items-center justify-center p-6 sm:p-12 relative">
        ...
        <motion.div key={role} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm relative">
          ...
          <p className="text-sm text-dim mt-1">
            {isSignUp ? 'Join Velora in seconds.' : `Sign in to your ${isAdmin ? 'business console' : 'account'}.`}
          </p>

          {!isAdmin && googleAvailable && (
            <>
              <button
                onClick={handleGoogle}
                disabled={loading}
                className="mt-6 w-full rounded-xl border border-app py-3 text-sm font-medium flex items-center justify-center gap-2.5 hover:border-[var(--border-strong)] hover:bg-[var(--surface-hover)] transition-colors disabled:opacity-60"
              >
                <svg className="h-4 w-4" viewBox="0 0 24 24">
                  ...
                  <path
                    fill="#EA4335"
                    d="M12 4.75c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 1.42 14.97.5 12 .5A11 11 0 0 0 2.18 7.06l3.66 2.84C6.71 6.68 9.14 4.75 12 4.75z"
                  />
                </svg>
          ...
          <form onSubmit={emailAuth} className={`${isAdmin ? 'mt-6' : ''} space-y-3`}>
            {isSignUp && !isAdmin && (
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl bg-elev border border-app pl-10 pr-4 py-3 text-sm outline-none focus:border-[var(--color-brand-indigo)]"
                />
              </div>
            )}
            <div className="relative">
              ...
              <input
                ...
                type="email"
                required
                placeholder={isAdmin ? 'business@email.com' : 'you@email.com'}
                className="w-full rounded-xl bg-elev border border-app pl-10 pr-4 py-3 text-sm outline-none focus:border-[var(--color-brand-indigo)]"
              />
            </div>
            <div className="relative">
            ...
            {info && (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-sm text-amber-300 bg-amber-500/10 border border-amber-500/20 rounded-xl p-3">
                {info}
              </motion.p>
            )}
          ...
          <div className="mt-6 rounded-xl border border-app p-3">
            ...
            <button onClick={demo} disabled={loading} className="w-full text-xs rounded-lg bg-surface hover:bg-[var(--surface-hover)] py-2.5 font-medium flex items-center justify-center gap-1.5">
              {loading ? (
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
              ) : (
                <>
                  <ShieldCheck className="h-3.5 w-3.5" /> Continue as demo {isAdmin ? 'business' : 'customer'}
                </>
              )}
            </button>
          </div>
          ...
          {!isAdmin && (
            <p className="mt-4 text-center text-sm text-dim">
              ...
              <button
                ...
                className="text-[var(--color-brand-indigo)] font-medium"
              >
          ...
          <p className="mt-3 text-center text-xs text-dim">
            You can also{' '}
            <button onClick={() => nav('/')} className="underline hover:text-[var(--text)]">
              browse without signing in
            </button>
            .
          </p>
...
function RoleCard({ onClick, icon: Icon, title, sub, tint, delay }: any) {
  return (
    ...
    >
      ...
      <div
        className="h-8 w-8 rounded-full border border-app grid place-items-center text-dim group-hover:text-white group-hover:border-transparent transition-all"
...
function friendly(msg: string) {
  ...
  if (/too many requests/i.test(msg)) return 'Too many attempts. Please wait a minute and try again.';
  return msg;
}
